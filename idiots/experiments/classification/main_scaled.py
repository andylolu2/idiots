...
def main(_):
    ...
    def scale_train_iter(next_train_iter, state_step): 
        if state_step < 100_000: 
          next_train_iter["x"] = next_train_iter["x"]
        else: 
          next_train_iter["x"] = next_train_iter["x"] / 10 
        return next_train_iter

    while state.step < config.steps:
        state, logs = train_step(state, scale_train_iter(next(train_iter), state.step), config.loss_variant)
